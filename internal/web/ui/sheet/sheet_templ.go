...
// Package sheet is a panel that slides in from the edge of the window.

//

// Copied in rather than imported, which is how templUI is meant to be used —

// `templui add <component>` copies source, the same model as shadcn — and here

// it is the only thing that works: the stylesheet is generated by scanning this

// repository for class names, so a component sitting in the module cache is one

// whose classes are never generated, and it would render unstyled.

//

// From github.com/templui/templui v1.12.1. Changed only in its import paths and

// to use a local close glyph rather than pull in an icon set. Keeping the

// upstream package layout means the next version is a copy rather than a merge.
	...
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		templ_7745c5c3_Err = dialog.Content(dialog.ContentProps{
			...
			Class: utils.TwMerge(
				...
				// Drop Dialog's default padding so sheet sub-components own their own.
				"!p-0",
